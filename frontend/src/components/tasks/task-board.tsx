...
type OwnerKey = keyof typeof TASK_CABANG;
...
const columns: Array<{
  ...
  {
    key: 'done',
    title: 'Sudah Check (Finish)',
    description: 'Setelah di-check, task di-finish dan hilang dari board ini',
    accent: 'border-emerald-300 bg-emerald-50/60',
    emptyText: 'Tarik task ke sini untuk finish',
  },
];

export default function TaskBoard({ cabang }: { cabang: OwnerKey }) {
  ...
  const [form, setForm] = useState<{ customer_id: string; project_id: string; title: string; type: string; priority: string; status: string; cabang: string; pic: string; deadline: string; estimate: string; notes: string }>({ customer_id: '', project_id: '', title: '', type: 'development', priority: 'medium', status: 'todo', cabang: cabang, pic: '', deadline: '', estimate: '', notes: '' });
  ...
  const fetchTasks = async () => {
    ...
    try {
      const params: Record<string, string | number> = { per_page: 100, cabang };
  ...
  useEffect(() => {
    fetchTasks();
  }, [search, priorityFilter, typeFilter, cabang]);
  ...
  useEffect(() => {
    ...
    void api.get<ApiResponse<Task>>(`/tasks/${drawer.id}`).then((response) => {
      ...
      setTask(value);
      setForm({ customer_id: value.customer_id ? String(value.customer_id) : '', project_id: value.project_id ? String(value.project_id) : '', title: value.title, type: value.type, priority: value.priority, status: value.status, cabang: value.cabang || cabang, pic: value.pic || '', deadline: value.deadline || '', estimate: value.estimate || '', notes: value.notes || '' });
    }).catch((err) => setError(err instanceof Error ? err.message : 'Detail task gagal dimuat.'));
  }, [drawer.id, cabang]);

  const openCreate = () => {
    setTask(null);
    setForm({ customer_id: '', project_id: '', title: '', type: 'development', priority: 'medium', status: 'todo', cabang: cabang, pic: '', deadline: '', estimate: '', notes: '' });
    drawer.open('create');
  };
  ...
  // Task yang selesai di-check (status done -> finished_at terisi) otomatis
  // hilang dari list utama karena backend menyembunyikan task finished.
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900">Task Board · {TASK_CABANG[cabang]}</h1>
          <p className="mt-2 text-sm text-slate-500">
            Seret task dari kiri ke tengah setelah dikerjakan, lalu ke kanan setelah di-check. Task yang sudah di-check tidak muncul lagi di sini.{' '}
            <Link href="/tasks" className="font-semibold text-blue-600 underline">Ganti owner</Link> ·{' '}
            <Link href="/tasks/all" className="font-semibold text-blue-600 underline">All Tasks</Link>
          </p>
        </div>
