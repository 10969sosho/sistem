...
export default function CustomersPage() {
  ...
  const remove = async (item: Customer) => {
    if (!window.confirm(`Hapus customer "${item.name}"? Seluruh project dan task terkait akan ikut terhapus.`)) return;
    setError('');
    try { await api.delete(`/customers/${item.id}`); await load(); }
    catch (err) { setError(err instanceof Error ? err.message : 'Customer gagal dihapus.'); }
  };
