  ...
  return (<div className="space-y-6">
    ...
    <Drawer open={drawer.mode !== null} title="Penawaran Baru" onClose={drawer.close}>
      <form onSubmit={(e) => { void submit(e); }} className="space-y-4">
        <FormField label="LEAD ID"><select className={fieldClass} value={form.lead_id} onChange={(e) => setForm({ ...form, lead_id: e.target.value })} required disabled={leadsLoading || leads.length === 0}><option value="">{leadsLoading ? 'Memuat leads...' : leads.length === 0 ? 'Belum ada lead' : 'Pilih lead'}</option>{leads.map((lead) => <option key={lead.id} value={lead.id}>{lead.name} (ID: {lead.id}){lead.company ? ` - ${lead.company}` : ''}</option>)}</select></FormField>
